              ...
              {menuItems.map((item) => {
                ...
                return (
                  <NavigationMenuItem key={item.label} className="relative group">
                    <Link
                      href={item.href}
                      className={`text-sm bg-transparent hover:bg-white/10 rounded-full px-4 py-2 font-hitmarker-text-medium text-[#999999] hover:text-vitalYellow transition-colors whitespace-nowrap duration-300 relative   hover:after:w-full flex items-center h-9 ${isActive ? "text-white after:w-full" : ""}`}u
                    >
                      {item.label}
                    </Link>
                  </NavigationMenuItem>
